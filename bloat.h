...
static uint64_t
align_mem(uint64_t pos, uint64_t size)
{
...
void *
arena_push(arena_t *arena, uint64_t size)
{
...
void *
arena_push_zero(arena_t *arena, uint64_t size)
{
...
void
arena_pop(arena_t *arena, uint64_t size)
{
...
void
arena_pop_to(arena_t *arena, uint64_t pos)
{
    if (pos >= arena->current->pos) return;
...
/* NOTE: temp arenas */
temp_arena_t *
temp_arena_alloc(arena_t *parent_arena)
{
...
static void
_temp_arena_restore(temp_arena_t *temp)
{
...
void
temp_arena_free(temp_arena_t *temp)
{
...
void *
temp_arena_push(temp_arena_t *temp, uint64_t size)
{
    return arena_push(temp->arena, size);
}

void
temp_arena_pop(temp_arena_t *temp)
{
    _temp_arena_restore(temp);
}
...
void
bloat_log(BLOAT_LOG type, char *fmt, ...)
{
...
void
sb_append(string_builder_t *sb, const char *item)
{
    if (!sb->items) _da_alloc(sb);
